...
typedef struct {
    char label[50];
    int address;        // address within code or data segment
...
/******************************************************************************
 * validate_brr and validate_mov (as in your original code)
 ******************************************************************************/
...
/******************************************************************************
 * PASS 1: Gather labels, track the program counter, and validate instructions.
 * Task 1 Modification: Use separate counters for code (.code) and data (.data)
 ******************************************************************************/
static void pass1(const char *filename) {
    ...
    codeCounter = 0x2000;   // code segment base address
    dataCounter = 0x10000;  // data segment base address
    ...
    while (fgets(line, sizeof(line), fin)) {
        ...
        // Process instructions (code) or data items.
        if (section == CODE) {
...
static void parseMacro(const char *line, FILE *fout) {
    ...
    if (!strcmp(op, "ld")) {
        ...
        /* Unrecognized macro line: output as-is */
        fprintf(fout, "\t%s\n", line);
    }
}

/******************************************************************************
 * Helper: Check if a line begins with a recognized macro.
 ******************************************************************************/
...
/******************************************************************************
 * PASS 2: Output file generation.
 * Task 1 Modification: Produce an output file starting with a header (of type
 * struct tinker_file_header) followed by the code segment and then the data segment.
 *
 * The header contains:
 *   u_int file_type;      // 0
 ...
 *
 * The assembler buffers the code and data lines into temporary files, then writes
 * the header and the buffered segments.
 ******************************************************************************/
static void pass2(const char *infile, const char *outfile) {
    ...
    /* Create temporary files to buffer code and data segments */
    ...
    while (fgets(line, sizeof(line), fin)) {
        ...
        if (line[0] == ':')  // skip label definitions
            continue;
        /* Replace label references (of the form "instruction:label") with the label's address */
        ...
        if (colon) {
            ...
            if (sscanf(colon + 1, "%49s", lbl) == 1) {
                ...
                if (entry) {
                    *colon = '\0'; // terminate instruction at colon
                    char buffer[1024];
    ...
    /* Append buffered code and data segments to the output file */
    rewind(codeTemp);
...
int main(int argc, char *argv[]) {
    ...
    /* PASS 1: Validate instructions and build label->address map using separate counters */
    pass1(argv[1]);
    /* PASS 2: Expand macros and replace label references, then produce final output */
    pass2(argv[1], argv[2]);
    free_hashmap();
    return 0;
}
